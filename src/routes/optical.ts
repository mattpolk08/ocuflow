...
opticalRoutes.post('/seed', async (c) => {
  try {
    await ensureOpticalSeed(c.env.OCULOFLOW_KV)
...
opticalRoutes.get('/inventory', async (c) => {
  try {
    const summary = await getInventorySummary(c.env.OCULOFLOW_KV)
...
opticalRoutes.get('/orders/summary', async (c) => {
  try {
    const summary = await getOrdersSummary(c.env.OCULOFLOW_KV)
    return c.json<ApiResp>({ success: true, data: summary })
  } catch (err) {
    return c.json<ApiResp>({ success: false, error: String(err) }, 500)
  }
...
opticalRoutes.get('/frames', async (c) => {
  try {
    ...
    let frames   = await listFrames(c.env.OCULOFLOW_KV)
...
opticalRoutes.get('/frames/:id', async (c) => {
  const frame = await getFrame(c.env.OCULOFLOW_KV, c.req.param('id'))
...
opticalRoutes.post('/frames', async (c) => {
  try {
    ...
    const frame = await createFrame(c.env.OCULOFLOW_KV, body)
...
opticalRoutes.patch('/frames/:id', async (c) => {
  try {
    ...
    const frame   = await updateFrame(c.env.OCULOFLOW_KV, id, updates)
    if (!frame) return c.json<ApiResp>({ success: false, error: 'Frame not found' }, 404)
...
opticalRoutes.get('/lenses', async (c) => {
  try {
    const q    = (c.req.query('q') ?? '').toLowerCase()
    let lenses = await listLenses(c.env.OCULOFLOW_KV)
...
opticalRoutes.get('/lenses/:id', async (c) => {
  const lens = await getLens(c.env.OCULOFLOW_KV, c.req.param('id'))
...
opticalRoutes.get('/contact-lenses', async (c) => {
  try {
    const q   = (c.req.query('q') ?? '').toLowerCase()
    let items = await listContactLenses(c.env.OCULOFLOW_KV)
...
opticalRoutes.get('/rx/patient/:pid', async (c) => {
  try {
    const rxList = await listRxForPatient(c.env.OCULOFLOW_KV, c.req.param('pid'))
...
opticalRoutes.get('/rx/:id', async (c) => {
  const rx = await getRx(c.env.OCULOFLOW_KV, c.req.param('id'))
...
opticalRoutes.post('/rx', async (c) => {
  try {
    ...
    const rx = await createRx(c.env.OCULOFLOW_KV, body)
...
opticalRoutes.get('/orders', async (c) => {
  try {
    ...
    let orders   = await listOrders(c.env.OCULOFLOW_KV)
...
opticalRoutes.get('/orders/:id', async (c) => {
  const order = await getOrder(c.env.OCULOFLOW_KV, c.req.param('id'))
...
opticalRoutes.post('/orders', async (c) => {
  try {
    ...
    const order = await createOrder(c.env.OCULOFLOW_KV, body)
...
opticalRoutes.post('/orders/:id/status', async (c) => {
  try {
    ...
    const result = await advanceOrderStatus(c.env.OCULOFLOW_KV, id, status, by, note)
...
opticalRoutes.patch('/orders/:id', async (c) => {
  try {
    const id      = c.req.param('id')
    const updates = await c.req.json()
    const order   = await updateOrderNotes(c.env.OCULOFLOW_KV, id, updates)
    if (!order) return c.json<ApiResp>({ success: false, error: 'Order not found' }, 404)
