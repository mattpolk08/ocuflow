...
scorecardsRoutes.get('/ping', async (c) => {
  await ensureScorecardseed(c.env.OCULOFLOW_KV)
...
scorecardsRoutes.get('/summary', async (c) => {
  try {
    ...
    const summary = await getPracticeSummary(c.env.OCULOFLOW_KV, range)
...
scorecardsRoutes.get('/providers/:id', async (c) => {
  try {
    const range = (c.req.query('range') || '30d') as DateRange
    const card  = await getProviderScorecard(c.env.OCULOFLOW_KV, c.req.param('id'), range)
...
scorecardsRoutes.get('/goals', async (c) => {
  try {
    ...
    const goals = await listGoals(c.env.OCULOFLOW_KV, providerId)
...
scorecardsRoutes.get('/goals/:id', async (c) => {
  try {
    const goals = await listGoals(c.env.OCULOFLOW_KV)
